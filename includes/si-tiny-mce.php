...
class SI_Tiny_MCE {
	...
	*/
	...
	if ( $teeny ) {
		...
		if($ideastream_editor_config && $ideastream_editor_config!="" && count($ideastream_editor_config)>0){
			if($ideastream_editor_config['image'] == 1 && $ideastream_editor_config['link']==0){
				$mce_buttons = apply_filters( 'teeny_mce_buttons', array('bold, italic, underline, blockquote, separator, strikethrough, bullist, numlist,justifyleft, justifycenter, justifyright, undo, redo, imathImage') );
			}
			elseif($ideastream_editor_config['image'] == 0 && $ideastream_editor_config['link']==1){
				$mce_buttons = apply_filters( 'teeny_mce_buttons', array('bold, italic, underline, blockquote, separator, strikethrough, bullist, numlist,justifyleft, justifycenter, justifyright, undo, redo, imathLink, unlink') );
			}
			elseif($ideastream_editor_config['image'] == 0 && $ideastream_editor_config['link']==0){
				$mce_buttons = apply_filters( 'teeny_mce_buttons', array('bold, italic, underline, blockquote, separator, strikethrough, bullist, numlist,justifyleft, justifycenter, justifyright, undo, redo') );
			}
			else{
	    ...
	    else $mce_buttons = apply_filters( 'teeny_mce_buttons', array('bold, italic, underline, blockquote, separator, strikethrough, bullist, numlist,justifyleft, justifycenter, justifyright, undo, redo, imathImage, imathLink, unlink') );
	...
	tinyMCEPreInit = {
		...
		mceInit : {<?php echo $mce_options; ?>,
			<?php if($ideastream_editor_config && $ideastream_editor_config!="" && count($ideastream_editor_config)>0):?>
		    setup : function(ed) {
		        // Add image button
				<?php if($ideastream_editor_config['link'] != 0):?>
		        ed.addButton('imathLink', {
		            title : '<?php _e("Add a link","wp-idea-stream");?>',
		            image : '<?php echo WP_IDEA_STREAM_PLUGIN_URL;?>/images/imath-link2.gif',
		            onclick : function() {
		                // Add you own code to execute something on click
						oldtext = ed.selection.getContent();
						if(!oldtext)
						{
							alert("<?php _e('Please select some content for your link','wp-idea-stream');?>");
							return;
						}else{
							siUrl = prompt("<?php _e('Add the url of your link','wp-idea-stream');?>", "http://");
							ed.focus();
				...
				<?php endif;?>
				<?php if($ideastream_editor_config['image'] != 0):?>
				ed.addButton('imathImage', {
		            title : '<?php _e("Add an image","wp-idea-stream");?>',
		            image : '<?php echo WP_IDEA_STREAM_PLUGIN_URL;?>/images/imath-image.gif',
		            onclick : function() {
		                // Add you own code to execute something on click
						img_post = prompt("<?php _e('Url to your image','wp-idea-stream');?>", "http://");
						if(img_post){
						img_width = prompt("<?php _e('Width in pixels of your image (only numbers)','wp-idea-stream');?>");
						<?php if(get_option('_ideastream_image_width')!=""):?>
							max_width_img = <?php echo get_option('_ideastream_image_width');?>;
						<?php else:?>
							max_width_img = 300;
						<?php endif;?>
